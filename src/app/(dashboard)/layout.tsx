...
const navItems = [
  ...
  { href: '/approvals', label: 'Approvals', icon: MessageSquare },
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className="fixed lg:static inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col -translate-x-full lg:translate-x-0 peer-checked:translate-x-0 transition-transform duration-200 ease-in-out">
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              prefetch={true}
              className="flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors"
            >
        ...
        <div className="border-t border-gray-200 p-4 space-y-3">
          <div className="px-3 space-y-2">
            <Suspense fallback={<div className="h-5 bg-gray-100 rounded animate-pulse" />}>
              <UserInfo />
            </Suspense>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="p-6 lg:p-8 lg:ml-0">
